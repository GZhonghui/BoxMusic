  ...
  "files": [
    {
      ...
      "name": "周杰伦 - 晴天.mp3",
      "artist": ["周杰伦"],           // 列表：0 到多个歌手，本地脚本按英文逗号分割
      "title": "晴天",                // 本地脚本解析得到
...
- 文件名解析（artist / title）由本地脚本完成，Web 应用直接用现成字段。`artist` 是**列表**（0 到多个歌手，脚本按英文逗号 `,` 分割）；`/视频` 一级目录下命名为「歌名 - 歌手」（反序），其余目录「歌手 - 歌名」。
